...
riders <- tribble(
  ...
  "Sofia Sjöborg", "Bryjamolga van het Marienshof Z", "belle", 386,
  "Sofia Sjöborg", "Targa D", "targa", 434
)
...
jump_clean <- jumps_raw |>
  mutate(
    ...
    across(
      c(lateral_balance, longitudinal_balance),
...
strides_clean <- strides_raw |>
  mutate(
    ...
    across(
      c(strike_power, lateral_balance, longitudinal_balance),
